...
## 预测步骤
### a、使用预训练权重
1. 下载完库后解压，下载训练权重YOLOX_s.pth，放入model_data，运行predict.py，输入  
```python
img/test.jpg
```
2. 在predict.py里面进行设置可以进行fps测试。  
### b、使用自己训练的权重
1. 按照训练步骤训练。  
...
img/test.jpg
```
4. 在predict.py里面进行设置可以进行fps测试。  

## 评估步骤 
1. 使用VOC格式进行评估。  
2. 如果在训练前已经运行过voc_annotation.py文件，代码会自动将数据集划分成训练集、验证集和测试集。
